...
const ShopInbox = () => {
  ...
  useEffect(() => {
    const getConversation = async () => {
      await axios
        .get(
          `${BASE_URL}/conversation/get-all-conversation-user/${user?._id}`,
          {
...
const MessageList = ({
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      try {
        const res = await axios.get(`${BASE_URL}/shop/shop-info/${userId}`, {
          withCredentials: true,
        });
...
const ChatBox = ({
  ...
  return (
    <div className="flex flex-col h-[90vh] w-[100%] sm:w-full rounded-lg">
      ...
      {/* messages section */}
      <div className="overflow-y-auto flex-1 h-[65vh] py-3 px-3 ">
        {messages &&
                ...
                >
                  ...
                  <div>
                    {item.sender !== userId ? (
                      ...
                      <div className="w-auto rounded-md p-3 mb-2 bg-slate-500 text-white">
                        <p className="w-[]">{item.text}</p>
                      </div>
                    )}
